...
1. 首次进入，没有数据展示，可以选择使用默认数据进行使用，查看整体展示效果。推荐导入自己的数据来进行操作。步骤如下：
    ...
    提示：若有显示图片头像的需求，可以在excel表格中填入url链接（https://）。若使用url链接不方便，可以在excel变革中填入头像图片的名称，然后将所有图片打包为zip压缩包传，在图片管理中上传头像的压缩包，随后就可以在人员列表中看到头像正常显示了，在界面设置中打开了“显示头像”，那么在首页也会看到头像显示。
